...
module.exports = function(){
	...
	const logger = loggerFactory.getLogger('PaymentProcessing', 'system');
...
function SetupForPool(poolOptions, setupFinished){
	...
	const logger = loggerFactory.getLogger('PaymentProcessing', coin);
    ...
    if (minConfPayout  < 3) {
        logger.warn(logComponent + ' minConf of 3 is recommended.');
    }
    ...
    if (parseInt(processingConfig.paymentInterval) < 120) {
        logger.warn(logComponent + ' minimum paymentInterval of 120 seconds recommended.');
    }
    ...
    logger.debug(logComponent + ' requireShielding: ' + requireShielding);
    logger.debug(logComponent + ' minConf: ' + minConfShield);
    logger.debug(logComponent + ' payments txfee reserve: ' + fee);
    ...
    function validateAddress (callback){
        daemon.cmd('validateaddress', [poolOptions.address], function(result) {
            if (result.error){
                ...
                logger.error('Daemon does not own pool address - payment processing can not be done with this daemon, '
    ...
    function listUnspent (addr, notAddr, minConf, displayBool, callback) {
        ...
        daemon.cmd('listunspent', args, function (result) {
            if (!result || result.error || result[0].error) {
                ...
                if (displayBool === true) {
                    logger.special(addr+' balance of ' + tBalance);
                }
    ...
    function listUnspentZ (addr, minConf, displayBool, callback) {
        daemon.cmd('z_getbalance', [addr, minConf], function (result) {
            if (!result || result.error || result[0].error) {
                ...
                if (displayBool === true) {
                    logger.error(addr.substring(0,14) + '...' + addr.substring(addr.length - 14) + ' balance: '+(zBalance).toFixed(8));
                }
    ...
    function sendTToZ (callback, tBalance) {
        ...
        if (opidCount > 0) {
            logger.warn('sendTToZ is waiting, too many z_sendmany operations already in progress.');
            return;
        }
        ...
        daemon.cmd('z_sendmany', params,
            function (result) {
                ...
                if (!result || result.error || result[0].error || !result[0].response) {
                    ...
                    opidCount++;
                    opids.push(opid);
                    logger.special('Shield balance ' + amount + ' ' + opid);
                    callback = function (){};
                    callback(null);
                }
            }
    ...
    function sendZToT (callback, zBalance) {
        ...
        // do not allow more than a single z_sendmany operation at a time
        if (opidCount > 0) {
            logger.warn('sendZToT is waiting, too many z_sendmany operations already in progress.');
            return;
        }
        ...
        daemon.cmd('z_sendmany', params,
            function (result) {
                ...
                if (!result || result.error || result[0].error || !result[0].response) {
                    ...
                    opidCount++;
                    opids.push(opid);
                    logger.special('Unshield funds for payout ' + amount + ' ' + opid);
                    callback = function (){};
                    callback(null);
                }
            }
    ...
    function cacheMarketStats() {
        ...
        request('https://api.coinmarketcap.com/v1/ticker/'+coin+'/', function (error, response, body) {
            if (error) {
                logger.error('Error with http request to https://api.coinmarketcap.com/ ' + JSON.stringify(error));
                return;
            }
            ...
            var checkOpIdSuccessAndGetResult = function(ops) {
                ...
                if (ops.length == 0) {
                    ...
                    if (opidCount !== 0) {
                        ...
                        opidCount = 0;
                        opids = [];
                        logger.warn('Clearing operation ids due to empty result set.');
                    }
                }
                ...
                ops.forEach(function(op, i){
                    ...
                    if (op.status == "success" || op.status == "failed") {
                        ...
                        if (op.status == "failed") {
                            ...
                            logger.special('Shielding operation success ' + op.id + '  txid: ' + op.result.txid);
                        }
                    } else if (op.status == "executing") {
                        logger.special('Shielding operation in progress ' + op.id );
                    }
                });
                ...
                daemon.batchCmd(batchRPC, function(error, results){
                    ...
                    results.forEach(function(result, i) {
                        if (result.result[i] && parseFloat(result.result[i].execution_secs || 0) > shielding_interval) {
                            logger.warn('Warning, walletInverval shorter than opid execution time of '+result.result[i].execution_secs+' secs.');
                        }
                    });
            ...
            daemon.cmd('z_getoperationstatus', null, function (result) {
                ...
                if (err === true) {
                    ...
                    if (opidCount !== 0) {
                        opidCount = 0;
                        opids = [];
                        logger.warn('Clearing operation ids due to RPC call errors.');
                    }
                }
            }, true, true);
        }
                            ...
                            blocks.forEach(function(block, i) {
                                if (block && block.result) {
                                    ...
                                    if (block.result.confirmations < 0) {
                                        logger.warn('Remove invalid duplicate block ' + block.result.height + ' > ' + block.result.hash);
                                        ...
                                        if (validBlocks.hasOwnProperty(dups[i].blockHash)) {
                                            ...
                                            logger.warn('Remove non-unique duplicate block ' + block.result.height + ' > ' + block.result.hash);
                                            // move from blocksPending to blocksDuplicate...
                                            invalidBlocks.push(['smove', coin + ':blocksPending', coin + ':blocksDuplicate', dups[i].serialized]);                                            
                                        } else {
                ...
                daemon.batchCmd(batchRPCcommand, function(error, txDetails){
                    ...
                    txDetails.forEach(function(tx, i){
                        ...
                        if (tx.error && tx.error.code === -5){
                            logger.warn('Daemon reports invalid transaction: ' + round.txHash);
                            round.category = 'kicked';
                            return;
                        }
                        else if (!tx.result.details || (tx.result.details && tx.result.details.length === 0)){
                            logger.warn('Daemon reports no details for transaction: ' + round.txHash);
                            round.category = 'kicked';
                            return;
                        }
                                            ...
                                            if (pplntEnabled === true && maxTime > 0) {
                                                ...
                                                if (workerTimes[address] != null && parseFloat(workerTimes[address]) > 0) {
                                                    ...
                                                    if (timePeriod > 0 && timePeriod < pplntTimeQualify) {
                                                        ...
                                                        sharesLost += lost;
                                                        shares = Math.max(shares - lost, 0);
                                                        logger.warn('PPLNT: Reduced shares for '+workerAddress+' round:' + round.height + ' maxTime:'+maxTime+'sec timePeriod:'+roundTo(timePeriod,6)+' shares:'+tshares+' lost:'+lost+' new:'+shares);
                                                    }
                                                    if (timePeriod > 1.0) {
                                                        err = true;
                        ...
                        if (result.error && result.error.code === -6) {
                            ...
                            if (result.error.message && result.error.message.includes("insufficient funds")) {
                                ...
                                if (tries < 5) {
                                    ...
                                    logger.warn('Insufficient funds (??) for payments ('+satoshisToCoins(totalSent)+'), decreasing rewards by ' + (higherPercent * 100).toFixed(1) + '% and retrying');
                                    trySend(higherPercent);
                                } else {
                                    logger.warn(rpccallTracking);
                            ...
                            if (txid != null) {
                                ...
                                if (withholdPercent > 0) {
                                    logger.warn('Had to withhold ' + (withholdPercent * 100)
            ...
            function(workers, rounds, paymentsUpdate, callback){
                ...
                var moveSharesToCurrent = function(r){
                    ...
                    if (workerShares != null) {
                        logger.warn('Moving shares from orphaned block '+r.height+' to current round.');
    ...
    var getProperAddress = function(address){
        if (address.length >= 40){
            logger.warn('Invalid address '+address+', convert to address '+(poolOptions.invalidAddress || poolOptions.address));
